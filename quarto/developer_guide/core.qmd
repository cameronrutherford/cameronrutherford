...
Since everything is built in this fasion, I support using a GitHub codespace to develop as well!

As the dev-container is re-built automatically in CI, you shouldn't have to re-build unless you are developing a new format or adding a new tool. If you ever do this, you can start rebuilding locally by adding this to the JSON config in place of `image`:

```JSON
    // If you want to build this container and debug locally, use this
     "build": {
       "dockerfile": "Dockerfile"
	},
```

Note that initially I had this as a comment, but JSON should not support comments. pre-commit checks caught this!
...
This is the current file in `.github/workflows/dev-container-build.yml` that I am using in generating the pre-build and publishing of a container to the registy:

```{.YAML include="../../.github/workflows/dev-container-build.yml"}
```

Inspired from the final example in this [Getting Started](https://github.com/devcontainers/ci/blob/main/docs/github-action.md#getting-started) section for usage of the Action.

TODO: Look into container features and ways to properly publish container image...
